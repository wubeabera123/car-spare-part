...
import { createProductAction, type ProductFormState } from "@/actions/seller-products";
...
export default function NewProductForm({
  ...
  return (
    <form action={action} className="mt-6 max-w-3xl space-y-5 rounded-2xl border border-border bg-surface p-6">
      {state?.message && (
      ...
      <div className="grid gap-4 sm:grid-cols-2">
        <Field name="name" label="Product name" error={err("name")} className="sm:col-span-2" />
        ...
        <div>
          ...
          <select id="partType" name="partType" className={selectCx} defaultValue="OEM">
        ...
        <div>
          ...
          <select id="condition" name="condition" className={selectCx} defaultValue="NEW">
            <option value="NEW">New</option>
            <option value="REFURBISHED">Refurbished</option>
            <option value="USED">Used</option>
          </select>
        </div>
        <Field name="price" label="Price (USD)" type="number" step="0.01" error={err("price")} />
        <Field name="compareAtPrice" label="Compare-at price (optional)" type="number" step="0.01" />
        <Field name="stock" label="Stock" type="number" defaultValue="1" error={err("stock")} />
      </div>
...
const selectCx = "mt-1.5 h-11 w-full rounded-lg border border-border bg-surface px-3 text-sm focus-ring";

function Label({ children, htmlFor }: { children: React.ReactNode; htmlFor: string }) {
  return <label className="text-sm font-medium" htmlFor={htmlFor}>{children}</label>;
}
function Hint({ children }: { children: React.ReactNode }) {
  return <p className="mt-1 text-xs text-accent-600">{children}</p>;
}
function Field({
  name, label, type = "text", step, defaultValue, className = "", error,
}: {
  name: string; label: string; type?: string; step?: string;
  defaultValue?: string; className?: string; error?: string;
}) {
  return (
